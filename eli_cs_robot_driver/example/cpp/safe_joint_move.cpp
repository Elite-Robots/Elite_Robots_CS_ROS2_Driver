...
public:
  ...
  SafeJointMoveNode()
  : rclcpp::Node("safe_joint_move_cpp")
  {
    target_ = this->declare_parameter<std::vector<double>>(
        "target", std::vector<double>{0.0, -1.2, 0.0, -1.57, 0.0, 0.0});
    ...
    client_ = rclcpp_action::create_client<FollowJT>(
        this, "/" + controller_ + "/follow_joint_trajectory");

    joint_sub_ = this->create_subscription<sensor_msgs::msg::JointState>(
        "/joint_states", 10,
        std::bind(&SafeJointMoveNode::jointCallback, this, std::placeholders::_1));
  }

  bool execute()
  {
    if (target_.size() != JOINTS.size()) {
      RCLCPP_ERROR(get_logger(), "Target size %zu does not match joint count %zu",
                   target_.size(), JOINTS.size());
      return false;
    }
    ...
    RCLCPP_INFO(get_logger(), "Max delta %.3f rad, duration %.3f s (max_vel %.3f rad/s)",
                max_delta, duration, max_vel_);
    ...
    if (result.result->error_code != 0) {
      RCLCPP_ERROR(get_logger(), "Controller returned error_code=%d",
